...
class AdType extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('place, description, insert_time, status', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('place, description', 'length', 'max'=>255),
			array('insert_time', 'length', 'max'=>11),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, place, description, insert_time, status', 'safe', 'on'=>'search'),
		);
	}
